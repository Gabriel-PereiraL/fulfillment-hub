...
## Fase 6 — Delivery provider simulator + integração de saída — `done`
...
**Resultado (2026-09-18)**: simulator `/delivery/v1` (token client-credentials fake com vida curta, quote/create/get/cancel com os nomes de campo do contrato de referência, erros `expired_quote`/`used_quote`/`duplicate_delivery`+metadata/`noncancelable_delivery`/`couriers_busy`/`customer_limited`, ciclo de vida temporizado com webhooks `event.delivery_status` assinados em `X-Uber-Signature` — só emitidos a partir da Fase 7; regras de sandbox por CEP); `IDeliveryProviderClient` com pipeline compartilhada (D-53) + token handler (D-54); D-P3 resolvida (D-51: cotação no checkout, taxa estimada como fallback); `RequestDeliveryHandler` no Worker (D-52) com recotação única (D-55), adoção em `409` (D-56), cancelamento no provider (D-57). Testes: 211 (11 unit do delivery client incl. CB abre **e fecha**; 10 integração de entrega). Gate 6 fechado; smoke em Kestrel (pedido → cotação R$17 → pago → entrega pedida pelo worker em ~7 s). Achado: hash de string randomizado por processo tornava a taxa do simulator não determinística (D-58).

## Fase 7 — Webhooks de entrega + idempotência + fora de ordem — `done`
...
**Resultado (2026-09-18)**: `POST /api/v1/webhooks/deliveries` sobre a pipeline compartilhada `WebhookReceiver` (D-60; header `X-Uber-Signature`, chave própria); `ApplyDeliveryWebhookHandler` + `DeliveryStatusApplier` com disposições do agregado e pedido acompanhando (D-61); `ReconcileDeliveriesHandler` + `DeliveryReconciliationService` (D-62, base `PeriodicJob`); simulator emitindo webhooks em Development e nos testes, CEP `…003` silencioso (D-63); D-59 (sem `GET` antes de aplicar entrega). Testes: 217 (6 novos: ponta a ponta pedido → entregue por webhooks reais, `returned` cancela + estoque, T7 fora de ordem + duplicado, assinatura/header/timestamp inválidos → 401, entrega desconhecida → `Ignored`, webhooks perdidos recuperados pela reconciliação). Gate 7 fechado; smoke em Kestrel: `AwaitingPayment → Paid → DeliveryRequested → InDelivery → Delivered` em 18 s com 4 webhooks aplicados.

## Fase 8 — Transactional outbox + Worker — `done`
...
**Resultado (2026-09-18)**: `outbox_messages` + `OutboxInterceptor` (eventos gravados no mesmo `SaveChanges`), `OutboxProcessor` (`FOR UPDATE SKIP LOCKED` + lease, escopo por mensagem, backoff exponencial com jitter, `Failed` após N, span ligado ao trace da request), `OutboxPublisherService` no Worker (500 ms), handlers `OrderPlaced` → pagamento, `OrderPaid` → entrega, `OrderCancelled`/`PaymentPaid` → estorno (BL-244 fechado), admin `GET/POST /api/v1/admin/outbox` (D-64…D-68); `POST /orders` responde `Created`; rate limit de webhooks configurável (D-69, achado na suíte). Testes: 222 (6 `OutboxTests`: T12 ×2, T13, admin só para Admin, redelivery idempotente, estorno por cancelamento; fixture hospeda o publisher pausável). Gate 8 fechado; ADR-004 com seção "Implementação"; smoke em Kestrel `Created → … → Delivered` em 18 s só pela outbox.
...
**Resultado (2026-09-18)**: `Messaging:Sqs` (D-70 chave de modo), `IAmazonSQS` + `SqsQueueProvisioner` (filas + DLQ com redrive, D-71), `IMessagePublisher`/`SqsMessagePublisher` (envelope JSON + `type`/`traceparent`), `OutboxProcessor` publicando em `fh-domain-events`, consumidores no Worker (`SqsConsumer` base com long polling, concorrência limitada, delete após sucesso, backoff por visibilidade D-75; `DomainEventsConsumer` com dedup `processed_messages` na mesma transação D-72; `WebhooksInboundConsumer`), webhooks via `fh-webhooks-inbound` com ponteiro + fallback in-process (D-73/D-74), `IWebhookProcessor` por provider, métricas `fh.queue.*`, instrumentação OTel do SDK, LocalStack no compose. Testes: 225 (3 `SqsMessagingTests` com Testcontainers LocalStack: fluxo pelas filas, T14 reentrega sem efeito duplo, T15 envenenada → DLQ após 3). Gate 9 fechado; ADR-005 com "Implementação"; smoke com compose completo: `Created → … → Delivered` em 16 s com webhooks aplicados pelo Worker.
