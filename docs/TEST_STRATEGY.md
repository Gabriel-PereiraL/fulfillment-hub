...
| T6 | Webhook duplicado | mesmo `event id` 2x → 200 ambos, um único efeito, métrica de duplicado | integration | 5/7 ✔ `DuplicateWebhook_IsAcknowledged_ButAppliedOnce` (1 linha em `webhook_events`, 1 transição `Paid`) |
| T7 | Webhook fora de ordem | `delivered` chega antes de `pickup` → estado vai a `Delivered` e o `pickup` posterior é registrado como `Stale` (não aplicado) | integration | 7 ✔ `Webhooks_OutOfOrder_NeverRegressTheDelivery_ButAreRecorded` (+ duplicado do mesmo `id` → 1 linha, 1 transição); ponta a ponta `Order_IsDelivered_ByTheProvidersWebhooks_EndToEnd`; `ReturnedParcel_CancelsTheOrder_AndReleasesStock`; `LostWebhooks_AreRecoveredByReconciliation`; assinatura errada / header do outro provider / timestamp velho → 401 |
...
| T10 | Circuit breaker | após N falhas abre; chamadas seguintes falham rápido (`BrokenCircuitException`); fecha após half-open bem-sucedido | unit/integration | 5/6 ✔ `CircuitBreaker_Opens_AfterSustainedFailures_AndFailsFastWithoutCallingTheProvider` (pagamento) + `CircuitBreaker_Opens_AfterSustainedFailures_AndClosesAgain_AfterASuccessfulProbe` (entrega: abre com 10 falhas, probe half-open após 1 s fecha o circuito) |
| T11 | Cotação expirada | `expires` no passado → recotar; 2ª expiração → falha de negócio | integration | 6 ✔ `RequestDelivery_WhenTheCheckoutQuoteExpired_RequotesOnce` (cotação sandbox de 1 s) e `RequestDelivery_WhenTheQuoteExpiresTwice_GivesUp_ForAnOperator` (provider forçado a `expired_quote` → `delivery.quote_expired`, pedido segue `Paid`) |
| T12 | Outbox perda zero | exceção injetada após `SaveChanges` e antes do publish → mensagem continua na outbox e é publicada pelo worker | integration | 8 ✔ `OrderPlaced_IsCommittedWithTheOrder_AndPublishedByTheWorker` (linha gravada no mesmo commit, nenhum efeito no request) + `PublishFailure_AfterTheCommit_IsRetried_UntilTheHandlerSucceeds` (provider fora na 1ª publicação → `Pending`, `attempts=1`, sucesso depois) |
| T13 | Outbox retry/Failed | handler falha 3x → `attempts=3`, `Failed`; reprocessar via endpoint → sucesso | integration | 8 ✔ `HandlerFailingRepeatedly_ParksTheMessageAsFailed_AndAnAdministratorCanRequeueIt`; `RedeliveringAProcessedMessage_HasNoSecondEffect` (consumidor idempotente); `CancellingAPaidOrder_RefundsThePayment_ThroughTheOutbox` |
| T14 | Consumidor idempotente | mesma mensagem SQS entregue 2x → um efeito | integration (LocalStack) | 9 ✔ `RedeliveredMessage_IsAcknowledged_WithoutASecondEffect` (1 pagamento, 1 linha em `processed_messages`) |
| T15 | DLQ | mensagem envenenada → DLQ após `maxReceiveCount` | integration (LocalStack) | 9 ✔ `PoisonMessage_LandsInTheDeadLetterQueue_AfterMaxReceiveCount` (3 recebimentos com backoff, depois na DLQ); `Order_IsPaidAndShipped_ThroughTheQueues` (fluxo pelas duas filas) |
