...
Written in Phase 0; sections 2–4 record what has been executed since. The code is published at https://github.com/Gabriel-PereiraL/fulfillment-hub (manual push, since 2026-09-18); images (Phase 13) and CI/SAST (Phase 14) are implemented and run on GitHub Actions; ECR push and AWS (Phases 15–16) remain future work.
...
| Workflow | Run | Result |
|---|---|---|
| CodeQL (SAST) | [35605030657](https://github.com/Gabriel-PereiraL/fulfillment-hub/actions/runs/35605030657) | **green** in 2 min 58 s; 63 C# rules; 1 finding (`cs/log-forging`, medium) triaged as a false positive with justification — SECURITY.md §6.4; 0 open alerts |
| CI | [35605030618](https://github.com/Gabriel-PereiraL/fulfillment-hub/actions/runs/35605030618) | build, format, vulnerable-package gate and gitleaks **green**; tests **230/231** — `PaymentFlowTests.PlaceOrder_InitiatesPayment_AndWebhookMarksOrderPaid` timed out waiting for `Paid` because the order was already `Delivered` (fast runner; the poller compared statuses for equality). Fixed in the test support (the poller now accepts a later happy-path status); reference run on the fix commit `0b2ecde`: [35606335238](https://github.com/Gabriel-PereiraL/fulfillment-hub/actions/runs/35606335238) — **green, 231/231** in 2 min 11 s; CodeQL [35606335294](https://github.com/Gabriel-PereiraL/fulfillment-hub/actions/runs/35606335294) green |

**Phases 11–14 remote gate (2026-09-21, commit `907478d`)** — CI [35623513605](https://github.com/Gabriel-PereiraL/fulfillment-hub/actions/runs/35623513605): locked-mode restore ✓, build 0 warnings ✓, 248 tests (245 passed, 3 E2E skipped in the unit/integration job) ✓, 0 vulnerable packages ✓, gitleaks ✓; **images job** 3 min 33 s: images built on the runner at 144/117/130 MB (api/worker/simulator) ✓, Trivy 0 HIGH/CRITICAL on all three ✓, stack ready after 4 s ✓, **E2E 3/3 against the containers** ✓, `down -v` removed containers, network and volumes ✓. CodeQL [35623513653](https://github.com/Gabriel-PereiraL/fulfillment-hub/actions/runs/35623513653) green (63 rules, the single already-triaged finding, 0 open alerts).

**After the rename to `fulfillment-hub` (2026-09-21, commit `b18b532`)** — CI [35632515830](https://github.com/Gabriel-PereiraL/fulfillment-hub/actions/runs/35632515830): `images` (Trivy pinned by digest, E2E 3/3) and gitleaks green on the first attempt; `build-test` failed once with **1/248** — `TraceContinuityTests` saw an `Outbox OrderPlaced` span on a foreign trace because `RunOnceAsync` drains the whole pending batch and an earlier test of the same collection had left an `OrderPlaced` behind — and was green on re-run of the same commit (test isolation, not a regression). Fixed in the test itself (the outbox is drained before the listener is registered; 102/102 integration tests locally). CodeQL [35632515885](https://github.com/Gabriel-PereiraL/fulfillment-hub/actions/runs/35632515885) green: 63 rules, 1 result (the already-dismissed `cs/log-forging`), 0 open alerts.
